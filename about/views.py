...
def about(request):
    try:
        ...
        if user_patient.exists():
            return render(request, 'about.html', {'email': email, 'user_as': 'patient', 'all_doctors' : all_doctors})
        if user_doctor.exists():
            return render(request, 'about.html', {'email': email, 'user_as': 'doctor', 'all_doctors' : all_doctors})
        if user_hospital.exists():
            return render(request, 'about.html', {'email': email, 'user_as': 'hospital', 'all_doctors' : all_doctors})
    except:
        email = None
